...
class EmailTemplateController extends Controller
{
    ...
    public function destroy($id)
    {
        ...
        $this->activity->record([
            'module'     => $this->module,
            'module_id'  => $email_template->id,
            'sub_module' => $email_template->name,
            'activity'   => 'deleted'
        ]);
    ...
    public function getContent($id)
    {
        ...
        if ($email_template->category === 'user') {
            $user = $this->user->findOrFail(request('user_id'));
        }

        if ($email_template->category === 'invoice' || $email_template->category === 'invoice-payment') {
            $invoice = $this->invoice->findByUuidOrFail(request('invoice_uuid'));
        }

        if ($email_template->category === 'quotation') {
            $quotation = $this->quotation->findByUuidOrFail(request('quotation_uuid'));
        }

        $mail_data = $this->repo->getContent([
            'template'  => $email_template,
            'user'      => isset($user) ? $user : '',
            'invoice'   => isset($invoice) ? $invoice : '',
            'quotation' => isset($quotation) ? $quotation : '',
        ]);
