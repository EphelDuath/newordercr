...
class MessageRepository
{
    ...
    public function store($params = array())
    {
        ...
        if (! $id) {
            $this->upload->store($this->module, $message->id, $upload_token);
        } else {
            $this->upload->update($this->module, $message->id, $upload_token);
        }

        $message->has_attachment = $this->upload->getAttachment($this->module, $message->id)->count() ? 1 : 0;
    ...
    public function reply($params = array())
    {
        ...
        $this->upload->store($this->module, $reply->id, $upload_token);

        $reply->has_attachment = $this->upload->getAttachment($this->module, $reply->id)->count() ? 1 : 0;
    ...
    {
        $this->upload->delete($this->module, $message->id);
        ...
        if ($message->Replies->count()) {
            ...
            $this->upload->bulkDelete($this->module, $sender_messages);

            $this->message->whereIn('id', $sender_messages)->update(['is_deleted_by_sender' => 2]);

            $receiver_messages = $message->Replies->where('to_user_id', '=', \Auth::user()->id)->pluck('id');

            $this->upload->bulkDelete($this->module, $receiver_messages);
