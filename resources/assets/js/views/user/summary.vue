        ...
        <div class="card">
            <div class="card-body">
                <center class="m-t-30"> <img :src="getAvatar(user)" class="img-circle" width="150" />
                    ...
                    <h6 class="card-subtitle" v-if="type === 'customer' && user.profile.company_id">{{user.profile.company.name}}</h6>
                    <h6 class="card-subtitle" v-if="type === 'staff' && user.profile.designation_id">{{user.profile.designation.name}}</h6>
...
<script>
    export default {
        ...
        methods: {
            ...
            updateStatus(status){
                axios.post('/api/user/'+this.type+'/'+this.user.id+'/status',{
                    status: status
                })
